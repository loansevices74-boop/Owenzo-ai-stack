...
def _build_report(phases, pf, ca, cg, vd_limit, length_m,
    ...
    pdf = FPDF()                      # plain FPDF - no custom class
    pdf.set_auto_page_break(auto=True, margin=18)
    pdf.add_page()

    # Header block (drawn manually)
    pdf.set_font("Helvetica", "B", 14)
    pdf.cell(0, 8, "WireSafe Design Report", align="C")
    pdf.ln(8)
    pdf.set_font("Helvetica", "", 9)
    pdf.cell(0, 5, f"Project: {project or '-'} | Client: {client or '-'} | Date: {rpt_date}", align="C")
    pdf.ln(7)
    ...
    # 5. Sign-off + Disclaimer + Footer line
    pdf.ln(4)
    ...
    pdf.cell(0, 5, "Owens U. Oriaikhi (COREN R72198) | WireSafe Design Aid", align="C")
...
if status == "PASS":
    ...
    if st.button("📄 Generate PDF Design Report"):
        try:
            ...
            st.error(f"PDF build failed: {type(e)._name_}: {e}")
else:
